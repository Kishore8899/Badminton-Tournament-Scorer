  ...
  return (
    <Card title="Tournament Setup">
      <div className="space-y-6">
        {step === 1 && (
          <div>
            <h3 className="text-lg font-semibold mb-4 text-brand-primary">Step 1: Basic Details</h3>
        ...
        {step === 2 && (
           <div>
            <h3 className="text-lg font-semibold mb-4 text-brand-primary">Step 2: Categories</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {allCategories.map(cat => (
                <label key={cat} className="flex items-center space-x-3 p-3 bg-brand-dark rounded-md cursor-pointer hover:bg-brand-dark/70">
                  <input
                    type="checkbox"
                    checked={categories.includes(cat)}
                    onChange={() => handleCategoryToggle(cat)}
                    className="h-5 w-5 rounded border-gray-500 bg-brand-secondary text-brand-primary focus:ring-brand-primary"
                  />
                  <span className="text-brand-light font-medium text-sm">{cat}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        {step === 3 && (
          <div>
            <h3 className="text-lg font-semibold mb-4 text-brand-primary">Step 3: Scoring Rules</h3>
